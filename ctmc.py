...
for ii in range(length(G0_vec)):
    ...
    Gf0 = math.ceil((1 - percent_male)*G0*eta*beta)
    ...
    #Time_data = NaN(maxiter,NumSim); % Initialize Time_data vector.
    Time_data = np.full(shape=(maxiter,NumSim), fill_value=np.nan)

    #Male_data = zeros(N,maxiter,NumSim); 
    Male_data = np.zeros((N, 2, 3)) 
    ...
    Female_data = np.zeros(N,maxiter,NumSim)
    ...
    #% "
    Ookinete_data = np.zeros(N,N,maxiter,NumSim)
    #Oocyst_data = zeros(N,N,maxiter,NumSim) 
    #% "
    
    #Sporozoite_data = zeros(N,N,maxiter,NumSim) 
    #% "
    Sporozoite_data = np.zeros(N,N,maxiter,NumSim)
    
    #Burst_time = zeros(N,N,maxiter,NumSim)
    #% "
    Burst_time = np.zeros(N,N,maxiter,NumSim)
